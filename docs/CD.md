...
| 조건 | 동작 |
|---|---|
| `main` 브랜치 push | 자동 실행 (10개 전체) |
| GitHub Actions 수동 실행 | `services` 입력에 서비스 이름을 공백으로 구분해 넣으면 해당 서비스만 빌드/배포. 비워두면 10개 전체 |
...
버전을 올릴 때는 `JMX_VERSION`과 `JMX_SHA256` 두 값을 함께 변경해야 합니다. 새 버전의 JAR를 직접 다운로드한 뒤 해시를 구해서 채워야 합니다. 워크플로우 주석에 명령어가 있습니다.
