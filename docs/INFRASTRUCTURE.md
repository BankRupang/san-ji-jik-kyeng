...
**Private Subnet 서비스와의 내부 통신**: ECS 태스크(Public Subnet)에서 RDS, Redis(Private Subnet)로 접근할 때는 NAT가 전혀 필요 없습니다. 같은 VPC 안에 있는 서브넷끼리는 VPC 라우터가 내부망으로 직접 연결하므로, 인터넷을 한 번도 거치지 않습니다. NAT는 오직 "Private Subnet 서버가 인터넷 밖으로 나갈 때"만 필요한 중계기입니다. RDS, Redis는 외부로 나갈 일이 없으므로 NAT 없이도 ECS 태스크와 문제없이 통신합니다. 접근 제어는 보안 그룹이 담당해 ECS 태스크에서 오는 요청만 허용하고 나머지는 전부 차단합니다.

| 트래픽 방향 | 경로 | NAT 필요 여부 |
| --- | --- | --- |
| ECS(Public) → RDS/Redis(Private) | 같은 VPC 내부, VPC 라우터 경유 | 불필요 |
| ECS(Public) → ECR/외부 API | 인터넷 게이트웨이로 직접 아웃바운드 | 불필요 (Public IP 사용) |
| RDS/Redis(Private) → 외부 | 해당 없음 (아웃바운드 없음) | 해당 없음 |
...
## 의사결정: 인증 서버 (Cognito vs Keycloak) ✅

**결론: Keycloak을 ECS Fargate 위에 직접 운영**

| 선택지 | 개념 | 장점 | 단점 |
| --- | --- | --- | --- |
| **Keycloak (자체 호스팅)** ✅ | 오픈소스 IAM, Fargate 위 컨테이너로 운영 | 벤더 종속성 없음, 커스텀 인증·권한 로직 자유롭게 구현, OAuth2/OIDC/SAML 표준 프로토콜 지원 | 직접 운영, 장애 시 로그인 전체 불가 |
| AWS Cognito ❌ | AWS 관리형 인증 서비스 | 별도 운영 불필요, AWS 연동 간편 | MAU(월 활성 사용자) 기반 과금, 커스텀 인증 흐름 제한적, AWS 종속 |

**Cognito 대신 Keycloak을 선택한 이유 세 가지**

1. **벤더 종속성 회피**: Cognito는 AWS 전용 API라 다른 클라우드나 환경으로 이전할 때 인증 체계를 통째로 교체해야 합니다. Keycloak은 OAuth2, OIDC 표준 프로토콜을 쓰므로 나중에 어느 환경에든 그대로 붙일 수 있습니다.

2. **커스텀 인증·권한 로직**: 경매 플랫폼 특성 상 사용자 역할(판매자/구매자/관리자)에 따른 세밀한 접근 제어가 필요합니다. Keycloak은 Realm, Client, Role, Group을 자유롭게 설계할 수 있고, 커스텀 인증 흐름(Authentication Flow)도 구현 가능합니다. Cognito는 이런 유연성이 제한적입니다.

3. **표준 프로토콜 지원**: Keycloak은 OAuth2, OIDC, SAML 2.0을 모두 지원합니다. Spring Security와의 연동이 표준 방식이라 특정 AWS SDK 의존성이 생기지 않습니다.

**ECS 배치 이유**: Keycloak을 별도 EC2에 올리면 인스턴스 비용이 상시 발생합니다. ECS Fargate에 올리면 태스크 단위 과금이라 상대적으로 저렴하고, 헬스체크·자동 재기동 같은 ECS 운영 이점도 그대로 누립니다.

**미채택**

- **Cognito**: MAU 기반 과금 구조가 사용자 증가와 함께 예측하기 어렵게 올라갈 수 있고, 커스텀 인증 흐름에 한계가 있습니다. AWS 벤더 종속도 큰 부담입니다.
...
**결론: 일반 App 서비스는 FARGATE_SPOT 허용, bid와 Keycloak은 On-Demand 고정**

Fargate Spot은 AWS가 여유 용량을 싸게 파는 방식입니다. 같은 스펙의 태스크를 On-Demand 대비 최대 70% 저렴하게 쓸 수 있지만, AWS가 용량이 부족해지면 2분 예고 후 강제로 태스크를 회수합니다. 따라서 갑자기 꺼져도 괜찮은 서비스에만 적용합니다.

**서비스별 전략**

| 서비스 | 전략 | 이유 |
| --- | --- | --- |
| App 9개 (config, discovery, gateway, user 등) | FARGATE base=1 + FARGATE_SPOT weight=3 | HTTP 짧은 연결. Spot 회수 시 ECS가 새 태스크를 자동으로 교체. desired=1이면 base=1로 On-Demand 유지, 스케일아웃 시 추가 태스크는 Spot |
| bid | launch_type=FARGATE (On-Demand 고정) | WebSocket 장기 연결. Spot 회수 시 STOMP 세션이 전부 끊겨 입찰 실패 발생 |
| Keycloak | launch_type=FARGATE (On-Demand 고정) | 인증 서버. 태스크가 꺼지면 로그인 전체 불가 |

**가중치 계산 예시**

`FARGATE base=1 weight=1` + `FARGATE_SPOT weight=3`으로 설정하면:
- 첫 번째 태스크: 반드시 FARGATE (base=1)
- 두 번째 이후 태스크: FARGATE_SPOT 3 : FARGATE 1 비율로 배분

desired_count=1인 서비스는 base=1이 적용되어 항상 On-Demand로 뜹니다. 향후 스케일아웃이 발생하는 서비스에서 추가 태스크부터 Spot 절감 효과가 납니다.

**코드 반영**

`modules/ecs/main.tf`의 `aws_ecs_cluster_capacity_providers`에 두 전략을 함께 선언하고, `aws_ecs_service.app`에서 `launch_type`을 제거하고 서비스 레벨 `capacity_provider_strategy` 블록으로 교체했습니다. bid와 keycloak은 `launch_type = "FARGATE"` 명시를 유지해 클러스터 기본 전략을 우회합니다.

## 의사결정: bid 서비스 독립 분리 ✅

**결론: bid를 별도 ECS 서비스로 분리**

bid는 다른 App 서비스와 특성이 달라서 같은 태스크로 묶으면 스케일링 단위가 얽히고 메모리 요건도 충족하기 어렵습니다.

| 구분 | App 서비스 (user, auction 등) | bid 서비스 |
| --- | --- | --- |
| 연결 방식 | HTTP (짧은 수명) | WebSocket (장기 연결, STOMP) |
| 트래픽 패턴 | 비교적 평탄 | 마감 직전 집중 급증 |
| 메모리 요건 | 서비스별 상이 | 2 vCPU / 8GB (STOMP 세션 수 비례) |
| 스케일링 | 고정 | Auto Scaling 2~6 태스크 |

**bid 분리로 얻는 이점**

- Auto Scaling 정책을 bid에만 독립적으로 적용합니다.
- App 서비스 배포가 bid 태스크에 영향을 주지 않습니다. (독립 롤링 배포)
- 메모리 사양을 bid 특성(STOMP 세션 다수 유지)에 맞게 따로 설정합니다.

코드에서는 ECS 코드 내에 `aws_ecs_task_definition.bid`, `aws_ecs_service.bid`로 다른 App 서비스와 별도 리소스로 정의되어 있고, CPU 60% 목표의 오토스케일링(`bid_min_capacity=2`, `bid_max_capacity=6`, `bid_cpu_target=60`)이 적용되어 있습니다.
...
KRaft은 Raft 합의 알고리즘을 사용합니다. Raft는 과반수 동의가 있어야 진행되므로, 1대 장애를 허용하려면 남은 2대가 과반수(2/3)를 형성해야 해 최소 3대가 필요합니다.
(2대로 구성하면 1대 다운 시 과반수를 만들 수 없어 클러스터 전체가 멈춥니다)
MVP에서는 EC2 다중화 비용과 관리 부담을 피하기 위해 단일 브로커를 선택했으나, 1대 장애가 곧 코어 비즈니스 중단을 의미하므로 3-브로커 구성으로 선회했습니다.
