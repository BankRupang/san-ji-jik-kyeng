...
- **Docker Hub**: 외부망 의존과 인증, rate limit 부담
...
```mermaid
flowchart TD
    push[git push / PR merge] --> build

    subgraph GHA["GitHub Actions"]
        build["gradle build<br>(테스트 포함)"]
        build --> docker["docker build<br>이미지 생성"]
        docker --> ecr["ECR push<br>(IAM OIDC 인증)"]
        ecr --> branch{배포 대상}
    end

    branch -->|App, Bid| ecs
    branch -->|Kafka, 모니터링| ssm

    subgraph ECS_DEPLOY["ECS Fargate 배포"]
        ecs["aws ecs update-service<br>--force-new-deployment"]
        ecs --> rolling["ECS 롤링 배포<br>구 태스크 순차 교체"]
        rolling --> pull_ecs["신 태스크가 ECR에서<br>이미지 pull (IAM Task Role)"]
    end

    subgraph EC2_DEPLOY["EC2 + Compose 배포"]
  ...
  **고도화 단계 추가** (초록색)
    ...
    graph TB
        ...
        subgraph CICD["GitHub Actions CI/CD"]
            Build["gradle build<br>docker build"]:::base
            ECRPush["ECR push<br>IAM OIDC"]:::base
            DeployECS["ECS rolling deploy"]:::base
            DeployEC2["SSM Run Command<br>compose pull && up"]:::base
        end
