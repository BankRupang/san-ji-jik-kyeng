...
public interface PaymentOutboxJpaRepository extends JpaRepository<PaymentOutbox, UUID> {
    ...
    @Query("SELECT o FROM PaymentOutbox o WHERE o.status = 'PENDING' OR (o.status = 'FAILED' AND o.retryCount < 3) ORDER BY o.createdAt ASC")
    List<PaymentOutbox> findRetryableOutboxes(Pageable pageable);
}
