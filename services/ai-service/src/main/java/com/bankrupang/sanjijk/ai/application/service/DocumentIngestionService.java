...
public class DocumentIngestionService {
    ...
    public void ingest(String title, String content, String source) {
        try {
            String prefixed = "title: " + title + " | text: " + content;
            Document doc = new Document(prefixed, Map.of("title", title, "source", source));
            vectorStore.add(List.of(doc));
            log.info("문서 적재 완료 - source: {}, title: {}", source, title);
        } catch (Exception e) {
            log.error("문서 적재 실패 - source: {}, title: {}, error: {}", source, title, e.getMessage());
